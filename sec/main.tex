...
(1) \textit{Image and Language Prompting for Critical Thinking}. We adopt the same image and language prompts as I-CONECT to stimulate participants' interests and cognitive function. The prompts include topics selected in the I-CONECT study that initiate conversations with minimal motivation required.
...
(4) \textit{Safety Assurance}. In contrast to the human-delivered protocol, we add safety assurance to define prohibited behaviors that should be avoided in conversations, for example, generating misinformation, financial advice, or harmful content. \comm{more details?}
...
\item
  ...
  \item
    ...
    twice per week, per participant self-report
  \end{enumerate}
\item
...
\item
  ...
  week); unstable dosing of cholinesterase inhibitors (must be stable for
  2 months).
\end{enumerate}
