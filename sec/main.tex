...
persistent barrier to broad dissemination (scalability).
...
\textbf{ADRD is prevalent and is associated with social isolation but remains difficult to treat.}
Alzheimer's disease and related dementias (ADRD) remain difficult to
treat, and available pharmacologic options are limited by side effects,
high cost, and restricted eligibility for newer disease-modifying
therapies. As millions of patients remain without accessible treatment
...
epidemiological research has repeatedly associated social isolation with
...
However, the critical barrier is not whether social connection matters,
but how to deliver sustained, high-fidelity intervention at
population scale for older adults at risk.

\textbf{The efficacy of conversational intervention against social isolation and dementia has been proven, but its scalable delivery remains challenging.}
...
AI-CONECT is innovative in directly translating a proven, interviewer-led dementia prevention intervention into a protocol-faithful, AI-delivered system tailored for socially isolated older adults. Rather than designing a de novo chatbot, we explicitly encode and iteratively refine the established I-CONECT workflow, conversational strategies, and cognitively demanding prompts within a multimodal, safety-aware AI framework. This systematic translation enables high-fidelity automation of a complex behavioral intervention while preserving its therapeutic intent, thereby opening a new paradigm for scaling evidence-based conversational therapies.

\textbf{Technology Innovation 1: Accelerating cohort-oriented development of AI chatbots via data-driven simulation.} 
Developing an AI chatbot for a cohort with specific needs (e.g., socially isolated older adults with mild cognitive impairment (MCI)) is limited by scarce data, high evaluation cost, and the risk of deploying immature designs. We introduce a data-driven simulation framework to model cohort-typical language and engagement behaviors and to stress-test candidate chatbot behaviors \emph{before} involving older adults. Unlike previous simulation methods\comm{cite Patient Psi}, our method is grounded in recorded I-CONECT conversations (enabling us to capture unique MCI traits) and intervention goals (e.g., engagement and reminiscence). This approach refines and extends existing chatbot development methods by adding quantitative, intervention-relevant evaluation environment that enable rapid iteration, transparent comparison across versions, and earlier identification of failure modes (e.g., low engagement or protocol drift), thereby reducing development time and risk.
...
Using proper prompts/cues can effectively draw their attention and stimulate cognitive functions \comm{cite BCT papers}.
...
\comm{Add a teaser figure.}

\textbf{C.1 Intervention Approach.}

The recent research, an Internet-based conversational engagement
...
project. Based on I-CONECT, we propose protocl of intervention and
verify the hypothesis through two-step evaluation: scalable virtual
testing and pilot human survey.

The intervention protocol is modified from I-CONECT to fit the AI-based intervention.
(1) Image and Language Prompts. We adopt the same image and language prompts as I-CONECT to stimulate participants' interests and cognitive functions.
(2) Reminiscence with Visual interactions. We will engage participants in reminiscence by asking
...
(3) Vocabulary Enhancement with Hints. We will prompt the participants to describe the presented images in detail to enhance their vocabulary usage. AI will guide the participants to describe specific details by providing visual hints, e.g., zooming in on image parts.
(4) Conversation Flow. Each session will last for 15 minutes, starting from greeting, image/topic presentation, open conversation where reminiscence and vocabulary enhancement will be executed spontaneously, and closing. \comm{Create a flow chart.}
...
\textbf{C.2 Development Approach.}

The key challenge is to implement the intervention strategies that have
...
\textbf{(2) Conversation Simulation for Implementation Validation}: To
...
We will test the chatbot on the stakeholders at the AITC and our
