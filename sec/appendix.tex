...
For identifiable data, which includes audio and video data, a data usage agreement (DUA) will be required. This DUA will be negotiated between the data applicant's institution and MGH. This step is necessary as video and audio data are considered identifiable and, when linked to clinical data, are classified as Protected Health Information (PHI). However, we are able to share the extracted feature variables from audio and video data, which do not contain any identifiable data, through platforms like Excel sheets as well as GitHub and other open sources.
...
At least, the study website will create a permanent link for the data request form along with the contact personnel information.
...
A Data Use Agreement (DUA) will be required to executed before accessing the data. 
...
\textbf{Oversight of data management and sharing:} The Principal Investigator for the project, Dr. Junyuan Hong, together with Dr. Liu Chen (data manager), Ms. Cathrine Young (the program manager), and a designated study coordinator, will oversee compliance with this plan, review data sharing readiness before release, and conduct compliance review annually.
